...
class LoginRequest extends FormRequest
{
    ...
    public function authenticate(): void
    {
        ...
        $user = \App\Models\User::where('email', $this->email)
            ->orWhere('username', $this->email) // Para username también
            ->first();

        if (! $user) {
            // Si el usuario no existe, mostrar error en el campo 'email'
            throw ValidationException::withMessages([
                'email' => 'El correo o nombre de usuario proporcionado no está registrado.',
            ]);
        }
        ...
        if (! \Illuminate\Support\Facades\Hash::check($this->password, $user->password)) {
            ...
            throw ValidationException::withMessages([
                'password' => 'Oops, parece que la contraseña no es correcta. Intenta nuevamente.',
            ]);
        }
